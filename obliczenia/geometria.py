...
def rysuj(odcinek1, odcinek2):
    ...
    if wspolrzedne:
        ...
        if len(wspolrzedne) == 4:
            ...
            plt.scatter(*poczatek, color="black", s=150, label=f"Początek zakresu nachodzenia ({poczatek[0]}, {poczatek[1]})")
            plt.scatter(*koniec, color="gray", s=150, label=f"Koniec zakresu nachodzenia ({koniec[0]}, {koniec[1]})")
            plt.plot([poczatek[0], koniec[0]], [poczatek[1], koniec[1]], label="Zakres nachodzenia", color="red", linestyle="--", linewidth=2)
            plt.title(f"Odcinki nachodzą na siebie od punktu ({poczatek[0]}, {poczatek[1]}) do punktu ({koniec[0]}, {koniec[1]}).")
        # Jeśli odcinki się przecinają, funkcja przeciecie zwróciła tylko współrzędne punktu przecięcia, czyli 2 liczby.
        else:
            plt.scatter(*wspolrzedne, color="red", marker="x", s=200, zorder=3, label=f"Punkt przecięcia ({wspolrzedne[0]:.2f}, {wspolrzedne[1]:.2f})")
            plt.title(f"Odcinki przecinają się w punkcie ({wspolrzedne[0]:.2f}, {wspolrzedne[1]:.2f})")
